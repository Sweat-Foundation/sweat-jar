...
fn run_end_to_end_writes_csv() {
    ...
    // account 700: deposit+apply_booster merged into one AirdropWithBooster —
    // confirms the merge replays end-to-end without the engine panicking and
    // reconciles cleanly (not a proof of exact interest math, just that the
    // whole pipeline accepts the merged action and produces a real claim).
    let row = body
        .lines()
        .find(|l| l.starts_with("700,"))
        .expect("row for 700");
    let cols: Vec<&str> = row.split(',').collect();
    assert_eq!(cols[7], "ok", "row: {row}");
    assert_eq!(cols[3], "456", "row: {row}");
}
...
fn run_shard_splits_worklist() {
    ...
    assert_eq!(s0.processed + s1.processed, 7);
    ...
    assert_eq!(ids.len(), 7, "each account reconciled exactly once across shards");
}
